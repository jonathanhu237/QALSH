...
extern uint64_t g_page_io; // global param: page i/o
    ...
    DType *data)        // data (return)
{
  ...
  switch (sign) {
  case 0:
    sprintf(fname, "%s.ds", prefix);
    break;
  case 1:
    sprintf(fname, "%s.q", prefix);
    break;
  case 2:
    sprintf(fname, "%s.gt%3.1f", prefix, p);
    break;
  default:
    ...
    const char *dfolder) // data folder
{
  ...
  memset(buffer, 0, B * sizeof(char)); // one page
  ...
  for (int i = 0; i < total_file; ++i) {
    ...
    char fname[200];
    sprintf(fname, "%s%d.data", dpath, i);
    ...
    DType *data)          // data point (return)
{
  ...
  char *buffer = new char[B];
  memset(buffer, 0, B * sizeof(char));
  char fname[200];
  sprintf(fname, "%sdata/%d.data", dfolder, fid);

  if (read_buffer_from_page(B, fname, buffer))
    return 1;
