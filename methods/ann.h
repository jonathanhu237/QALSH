    ...
    const char *ofolder) // output folder
{
  ...
  for (int top_k : TOPKs) {
    ...
    for (int i = 0; i < qn; ++i) {
      g_page_io += linear<DType>(n, d, B, p, top_k, &query[(uint64_t)i * d],
                                 dfolder, list);
    ...
    printf("%d\t\t%.4f\t\t%llu\t\t%.2f\t\t%.2f\n", top_k, g_ratio, g_page_io,
           g_runtime, g_recall);
    fprintf(fp, "%d\t%f\t%llu\t%f\t%f\n", top_k, g_ratio, g_page_io, g_runtime,
            g_recall);
  }
  printf("\n");
  fprintf(fp, "\n");
    ...
    const char *ofolder)    // output folder
{
  ...
  QALSH_PLUS<DType> *lsh =
      new QALSH_PLUS<DType>(n, d, B, leaf, L, M, p, zeta, c, data, path);
  lsh->display();
    ...
    const char *ofolder) // output folder
{
  ...
  for (int nb = 1; nb <= lsh->get_num_blocks(); ++nb) {
    ...
    for (int top_k : TOPKs) {
      ...
      for (int i = 0; i < qn; ++i) {
        g_page_io +=
            lsh->knn(top_k, nb, &query[(uint64_t)i * d], dfolder, list);
        g_ratio += calc_ratio(top_k, &truth[(uint64_t)i * MAXK], list);
        g_recall += calc_recall(top_k, &truth[(uint64_t)i * MAXK], list);
      }
      delete list;
