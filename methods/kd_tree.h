...
// -----------------------------------------------------------------------------
template <class DType> class KD_Rect {
public:
...
{
  for (int i = 0; i < dim; ++i) {
    if (point[i] < low_[i] || point[i] > high_[i])
      return false;
  }
  return true;
}
...
// -----------------------------------------------------------------------------
template <class DType> class KD_Tree {
public:
  KD_Tree(                // constructor
    ...
    KD_Rect<DType> &bnd_box) // bounding box for current node (return)
{
  if (n <= leaf_) {
    ...
    return new KD_Split<DType>(dim_, cut_dim, cut_val, low_val, high_val, left,
                               right, data_);
  }
}
