...
// -----------------------------------------------------------------------------
template <class DType> class QALSH {
public:
    ...
    const DType *data)      // data set
{
  ...
  for (int i = 0; i < m_; ++i) {
    ...
    if (trees_[i]->bulkload(n_pts_, table))
      return 1;
  }
  delete[] table;
  return 0;
  ...
  while (true) {
    ...
    while (num_flag < m_) {
      for (int i = 0; i < m_; ++i) {
        ...
        if (lptr->size_ != -1)
          ldist = calc_dist(q_val[i], lptr);
        if (rptr->size_ != -1)
          rdist = calc_dist(q_val[i], rptr);
        ...
        if (num_flag >= m_ || dist_io_ >= candidates)
          break;
      }
      if (num_flag >= m_ || dist_io_ >= candidates)
        break;
    ...
    if (kdist < c_ * radius && dist_io_ >= top_k)
      break;
    if (dist_io_ >= candidates)
      break;
    ...
    MinK_List *list)         // k-NN results (return)
{
  ...
  while (true) {
    ...
    // step 3: stop conditions 1 & 2
    if (dist_io_ >= candidates || num_range >= m_)
      break;
